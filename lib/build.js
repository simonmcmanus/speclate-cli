...
module.exports = (spec, speclate, callback) => {
  ...
  var methods = [
    next => {
      ...
      var options = validSpec.options
      delete validSpec.options
      var defaultSpec = validSpec.defaultSpec
      delete validSpec.defaultSpec
      validRoutes = validSpec;
    ...
    next => {
      ...
      completeSpec = combineSpecs(validRoutes, speclate, next)
      log.success('ok')
      next()
    },
    ...
    next => {
      log.info('writing required client side files')
      var folder = process.cwd() + '/docs/client'
      var fileName = folder + '/speclate-required-lists.mjs'
      log.info('writing' + fileName)

      mkdirp.sync(folder)
      fs.writeFile(fileName, 'export default ' + JSON.stringify(requiredFileNames.allFiles, null, 4), next)
    },
    next => {
    ...
    next => {
      ...
      generateMarkup(validSpec, speclate, files, (err, files) => {
        const generateSite = files
        next()
      })
    },
    next => {
      log.info('Creating layout')
      console.log(files.layout)
    ...
    next => {
      ...
      // takees options.files, and looks in scanSpecForFiles and moves them into the docs directory.
