...
module.exports = (spec, speclate, callback) => {
  ...
  var methods = [
    ...
    (next) => {
      ...
      validSpec = dynamicRoutes(spec, lists)
      log.success('ok')
      next()
    },
    (next) => { // generate offline spec.
      console.log('')
      console.log(chalk.blue('Generating list of files required for offline use..'))
      offlineSpec(spec, speclate, next)
