...
module.exports = function (spec, speclate, files, callback) {
  speclate.site(spec, files, function (error, files) {
    ...
    async.each(files, function (item, next) {
      ...
      var partPath = path.join(spec.options.outputDir, item.name.slice(0, -4) + 'mjs')
      log.debug(path.join(process.cwd(), partPath))
      writeFile(path.join(process.cwd(), partPath), 'export default ' + JSON.stringify(compiledSpec, null, 4), function (err) {
        if (err) {
          return next(err)
        }
