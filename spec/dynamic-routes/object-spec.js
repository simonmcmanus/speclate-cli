...
describe('given a spec with dynamic routes (object) ', function () {
  ...
  var spec = {
    '/tag/:tagname/index.html': {
      ...
      spec: {
        title: 'page title',
        '.links_holder': { component: 'link' }
      }
    }
  }
  describe('calling dynamic-routes', function () {
    var generatedRoutes = dynamicRoutes(spec)

    console.log('-obj', generatedRoutes);




    it('should take the urls from the data object')
    it('should use the spec for each route generated from the data object')
    it('should call any map files provided.')
